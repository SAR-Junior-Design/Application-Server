...
class Mission():
        ...
        if 'user' in session.keys():
            ...
            if User_DBModel.query.filter_by(id=user["id"]).first().account_type == "government_official":
                ...
                for mission in missions:
                    ...
                    drones = Drone_DBModel.query.join(Asset_DBModel).filter(
                        Drone_DBModel.id == Asset_DBModel.drone_id).filter(Asset_DBModel.mission_id == mission.id).all()
                ...
                commanded_missions = Mission_DBModel.query.filter(Mission_DBModel.commander == user['id']).filter(
                ...
                participating_missions = Mission_DBModel.query.join(Asset_DBModel,
                                                                    Mission_DBModel.id == Asset_DBModel.mission_id).filter(
                    Asset_DBModel.operator == user['id']).filter(Mission_DBModel.closed_at == None).all()
                for mission in participating_missions:
                    drones = Drone_DBModel.query.join(Asset_DBModel).filter(
                        Drone_DBModel.id == Asset_DBModel.drone_id).filter(Asset_DBModel.mission_id == mission.id).all()
    ...
    def get_missions():
        if 'user' in session.keys():
            ...
            if User_DBModel.query.filter_by(id=user["id"]).first().account_type == "government_official":
                missions = Mission_DBModel.query.filter(starts_at < Mission_DBModel.starts_at,
                                                        Mission_DBModel.ends_at < ends_at).all()
                ...
                for mission in missions:
                    ...
                    mission_list += [{'id': mission.id, 'title': mission.title,
                                      'description': mission.description, 'commander': commander.name,
                                      'commander_id': mission.commander,
                                      'starts_at': str(mission.starts_at), 'ends_at': str(mission.ends_at),
                                      'drones': drone_list, 'num_drones': len(drone_list),
                                      'clearance': mission.clearance,
                ...
                commanded_missions = Mission_DBModel.query.filter(Mission_DBModel.commander == user['id'],
                                                                  starts_at < Mission_DBModel.starts_at,
                                                                  Mission_DBModel.ends_at < ends_at).all()
                ...
                participating_missions = Mission_DBModel.query.join(Asset_DBModel,
                                                                    Mission_DBModel.id == Asset_DBModel.mission_id).filter(
                    Asset_DBModel.operator == user['id'], Mission_DBModel.commander != user['id'],
                    starts_at < Mission_DBModel.starts_at, ends_at > Mission_DBModel.ends_at).all()
                for mission in participating_missions:
                    drones = Drone_DBModel.query.join(Asset_DBModel).filter(
                        Drone_DBModel.id == Asset_DBModel.drone_id).filter(Asset_DBModel.mission_id == mission.id).all()
                    ...
                    mission_list += [{'id': mission.id, 'title': mission.title,
                                      ...
                                      'starts_at': str(mission.starts_at), 'ends_at': str(mission.ends_at),
                                      'drones': drone_list, 'num_drones': len(drone_list),
                                      'clearance': mission.clearance, 'type': mission.type}]
    ...
    def get_active_missions():
        if 'user' in session.keys():
            ...
            if User_DBModel.query.filter_by(id=user["id"]).first().account_type == "government_official":
                ...
                missions = Mission_DBModel.query.filter(Mission_DBModel.starts_at < current_time,
                                                        current_time < Mission_DBModel.ends_at).all()
                ...
                commanded_missions = Mission_DBModel.query.filter(Mission_DBModel.commander == user['id'],
                                                                  Mission_DBModel.starts_at < current_time,
                                                                  current_time < Mission_DBModel.ends_at).all()
                ...
                participating_missions = Mission_DBModel.query.join(Asset_DBModel,
                                                                    Mission_DBModel.id == Asset_DBModel.mission_id).filter(
                    Asset_DBModel.operator == user['id'], Mission_DBModel.commander != user['id'],
                    Mission_DBModel.starts_at < current_time, current_time < Mission_DBModel.ends_at).all()
                for mission in participating_missions:
                    drones = Drone_DBModel.query.join(Asset_DBModel).filter(
                        Drone_DBModel.id == Asset_DBModel.drone_id).filter(Asset_DBModel.mission_id == mission.id).all()
    ...
    def get_upcoming_missions():
        if 'user' in session.keys():
            ...
            if User_DBModel.query.filter_by(id=user["id"]).first().account_type == "government_official":
                ...
                commanded_missions = Mission_DBModel.query.filter(Mission_DBModel.commander == user['id'],
                                                                  Mission_DBModel.starts_at > current_time).all()
    ...
    def get_mission_info():
        if 'user' in session.keys():
            ...
            drones = Drone_DBModel.query.join(Asset_DBModel).filter(Drone_DBModel.id == Asset_DBModel.drone_id).filter(
                Asset_DBModel.mission_id == mission_id).all()
...
app.add_url_rule('/v1_0/add_drone_to_mission', '/v1_0/add_drone_to_mission', Mission.add_drone_to_mission,
                 methods=['POST'])
app.add_url_rule('/v1_0/remove_drone_from_mission', '/v1_0/remove_drone_from_mission',
                 Mission.remove_drone_from_mission, methods=['POST'])
app.add_url_rule('/v1_0/end_mission', '/v1_0/end_mission', Mission.end_mission, methods=['POST'])
app.add_url_rule('/v1_0/update_mission_area', '/v1_0/update_mission_area', Mission.update_mission_area,
                 methods=['POST'])
...
app.add_url_rule('/v1_0/edit_mission_details', '/v1_0/edit_mission_details', Mission.edit_mission_details,
                 methods=['POST'])
...
app.add_url_rule('/v1_0/get_passive_missions', '/v1_0/get_passive_missions', Mission.get_passive_missions,
                 methods=['GET'])
...
app.add_url_rule('/v1_1/get_possible_mission_conflicts', '/v1_1/get_possible_mission_conflicts',
                 Mission.get_possible_mission_conflicts, methods=['POST'])
app.add_url_rule('/v1_1/get_mission_info', '/v1_1/get_mission_info', Mission.get_mission_info, methods=['POST'])
app.add_url_rule('/v1_1/edit_mission_details', '/v1_1/edit_mission_details', Mission.edit_mission_details,
                 methods=['POST'])
app.add_url_rule('/v1_1/get_upcoming_missions', '/v1_1/get_upcoming_missions', Mission.get_upcoming_missions,
                 methods=['GET'])
